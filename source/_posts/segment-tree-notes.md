...
$\mathcal O(1)$ 是困难的，所以我们思考可不可以 $\boldsymbol{\mathcal O(\log n)}$ 实现。

## 分段

### 目标

我们可以把一个区间**分割成 $\boldsymbol{\mathcal O(\log n)}$ 个小区间**（这一点非常重要），如果每个区间都可以 $\boldsymbol{\Theta(1)}$ 完成（当然也很重要，但是如果是均摊 $\Theta(1)$ 也行），则总时间复杂度为 $\mathcal O(\log n)$。

### 较为失败的分段

我们的第一反应是把整个序列分割成几块，但是这样显然是不行的，哪里没割我们就可以把区间的一个端点设置为那个点，然后就**无法分割**了。如果把整个区间分割成每一个长度都是 $1$，则和数组无异，分割成的块数是 $\boldsymbol{\mathcal O(n)}$ 的。
...
这样做的好处：**一个数字 $\boldsymbol k$ 有 $\boldsymbol{\Theta(\log k)}$ 个二进制位**，完美符合我们“一个序列被分割成 $\mathcal O(\log k)$ 个小序列”的要求。
...
很不幸，把区间内所有数字加上某个数字的时间复杂度为 $\boldsymbol{\Theta(n \log n)}$，还不如直接在数组上操作呢。
...
事实上，专门用来记录这个值的变量叫做 $\textbf{lazytag}$，以下为了节省 $\text{KaTeX}$ 的码量，则把它称作 $\textbf{tag}$~~（$	ext{KaTeX}$ 中 `\text` 内怎么加粗啊？/fn）~~（使用 `\textbf` 即可）。
...
以此类推，时间复杂度：$T(n)=T(\frac{n}{2})+\Theta(1)$，显然 $T(n)=\Theta(\log n)$，也就是时间复杂度是 $\boldsymbol{\Theta(\log n)}$。

完美接招。

## hack $2$：右边少一个元素

完美接招，把左边的稍微改改，变成每次第一部分直接 $\Theta(1)$ 处理掉了，时间复杂度 $\boldsymbol{\Theta(\log n)}$。
...
然后 $2 \times \Theta(\log n) = \boldsymbol{\Theta(\log n)}$，又是一次完美地应对。
...
我们发现，一旦经过一次第 $8$ 中，就必然不会再经过第 $8,9$ 或 $10$ 种了。所以，如果我们把第 $1 \sim 7$ 种和第 $9,10$ 中当做两个组的话，那么 $8$ 就是连接这两个组的桥梁。其中，第一个组花费的时间是 $\mathcal O(\log n)$ 的，第二个组也是，而“桥梁”最多被调用一次，所以最终时间复杂度是 $\mathcal O(\log n) + \mathcal O(\log n) + 2\mathcal O(\log n) + \mathcal O(1) = \boldsymbol{\mathcal O(\log n)}$。

查询时间复杂度也是 $\boldsymbol{\mathcal O(\log n)}$ 的。
