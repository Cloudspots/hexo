---
title: 题解：CF2206E Parallel Sums
date: 2026-7-8 15:24:13
categories:
  - Solution
tags:
  - Solution
...
并不能通过（我不会回滚莫队）。如果有卡常大手子用这个卡过了请和我说一声。
...
[rec](https://codeforces.com/contest/2206/submission/381685862)。
...
class delheap { public: myheap has, del; void maintain() { while(!del.empty() && has.top() == del.top()) { has.pop(); del.pop(); }} void push(long long x) { if(!del.empty() && x == del.top()) del.pop(); else has.push(x); } bool empty() { return has.size() == del.size(); } void erase(long long x) { if(x == has.top()) has.pop(); else del.push(x); } long long top() { maintain(); return has.top(); } };
...
int main()
{
	...
	for(unsigned i=0;i<m;i++)
	{
		maxs[i].has.st.resize(n / m + 1);
		// maxs[i].has.st.push_back(0);
		maxs[i].del.st.resize(n / m + 1);
		// maxs[i].del.st.push_back(0);
	}
	// if(m == 1999)
	// {
	// 	long long s = 0;
